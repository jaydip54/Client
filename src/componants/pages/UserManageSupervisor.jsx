...
import { AddUserBySuperVisorEndpoint, AddUserEndpoint } from '../../Atoms/constatnt';

// Main component for User Management Admin page
const UserManageSupervisor = () => {
  ...
  const handleAddUser = async () => {
    try {
      if (!user.name || !user.email || !user.number || !user.password || !user.role) {
        setSnackbarMessage('All fields are required');
        setSnackbarSeverity('error');
        setOpenSnackbar(true);
        return;
      }
      const result = await dispatch(addUser({ data: user, endpoint: AddUserBySuperVisorEndpoint, token: token })).unwrap();
    ...
    <Box sx={{
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          ...
          >
             {role
              .filter(rolevalue => rolevalue.name === 'STAFF' || rolevalue.name === 'TECHNICIAN')
              .map((rolevalue) => (
...
export default UserManageSupervisor;
